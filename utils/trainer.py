...
class TrainingManager:
    ...
    @staticmethod
    def _restore_frozen_eval_mode(net):
        """Keep fully frozen submodules deterministic after net.train()."""
    ...
    def one_epoch(self, training=True):
        ...
        if training:
            net = self.net.train()
            self._restore_frozen_eval_mode(net)
        ...
        if max_batches is not None:
            print_freq = min(print_freq, max_batches)
        ...
        for it, batch in enumerate(loader):
            ...
            xyz = batch["xyz"].cuda(self.rank, non_blocking=True)
            ...
            if it % print_freq == print_freq - 1 or it == len(loader) - 1:
                ...
                if self.rank == 0 or self.rank is None:
                    ...
                    if (writer is not None) and (training or it == len(loader) - 1):
                        ...
                        z_aware_stats = getattr(model, "z_aware_stats", None)
                        if z_aware_stats is not None:
                            base_tokens = max(z_aware_stats["base_tokens"], 1)
                            writer.add_scalar(
                                header + "/prototype_extra_ratio",
                                z_aware_stats["split_tokens"] / base_tokens,
                                step,
                            )
                            writer.add_scalar(
                                header + "/prototype_vertical_entropy",
                                z_aware_stats["mean_entropy"],
                                step,
                            )
                        budgeted_router_stats = getattr(model, "budgeted_router_stats", None)
                        if budgeted_router_stats is not None:
                            fine_tokens = max(budgeted_router_stats["fine_tokens"], 1)
                            writer.add_scalar(
                                header + "/budgeted-router_token_ratio",
                                budgeted_router_stats["tokens"] / fine_tokens,
                                step,
                            )
                            writer.add_scalar(
                                header + "/budgeted-router_split_regions",
                                budgeted_router_stats["split_regions"],
                                step,
                            )
                            writer.add_scalar(
                                header + "/budgeted-router_vertical_std",
                                budgeted_router_stats["mean_vertical_std"],
                                step,
                            )
                            writer.add_scalar(
                                header + "/budgeted-router_feature_heterogeneity",
                                budgeted_router_stats["mean_feature_heterogeneity"],
                                step,
                            )
                            writer.add_scalar(
                                header + "/budgeted-router_selected_density",
                                budgeted_router_stats["selected_fine_density"],
                                step,
                            )
                            writer.add_scalar(
                                header + "/budgeted-router_merged_density",
                                budgeted_router_stats["merged_fine_density"],
                                step,
                            )
                        sparse_stats = getattr(model, "sparse_batch_reindex_stats", None)
                        if sparse_stats is not None:
                            padded_tokens = max(sparse_stats["padded_tokens"], 1)
                            writer.add_scalar(
                                header + "/sparse_valid_token_ratio",
                                sparse_stats["tokens"] / padded_tokens,
                                step,
                            )
                            writer.add_scalar(
                                header + "/sparse_padding_removed",
                                sparse_stats["padding_removed"],
                                step,
                            )
