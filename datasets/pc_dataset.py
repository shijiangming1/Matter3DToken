...
class PCDataset(Dataset):
    def __init__(
        ...
        num_instances=40,
        instance_cache_dir=None,
        prepare_instance_cache=False,
    ):
        super().__init__()
        ...
        self.prepare_instance_cache = prepare_instance_cache
    ...
    def __getitem__(self, index):
        ...
        out = (
            # Point features
            pc[:, 3:],
            # Point coordinates used by geometry-aware tokenizers
            pc[:, :3],
...
class Collate:
    def __init__(self, ignore_index=255, sparse_batch_reindex=False):
        self.ignore_index = ignore_index
        self.sparse_batch_reindex = sparse_batch_reindex

    def __call__(self, list_data):
        # Extract all data
        list_of_data = (list(data) for data in zip(*list_data))
        feat, xyz, labels, neighbors, idx_bev, xy_bev, upsample, filename = list_of_data

        if getattr(self, "sparse_batch_reindex", False):
            bev_lengths = [int(i.max()) + 1 for i in idx_bev]
            offsets = np.cumsum([0] + bev_lengths[:-1])
            for sample_idx, offset in enumerate(offsets):
                idx_bev[sample_idx] += offset
            splits_bev = bev_lengths
            idx_bev = torch.from_numpy(np.concatenate(idx_bev, 0))
            xy_bev = torch.from_numpy(np.concatenate(xy_bev, 0))
            idx_occ = torch.arange(sum(bev_lengths), dtype=torch.int32)
            idx_pad = None
        else:
            (
                idx_bev,
                ...
                idx_pad,
                splits_bev,
            ) = self._collate_padded_bev(idx_bev, xy_bev)

        # End of each point sequences
        scan_counts = [len(f) for f in feat]
        splits_pc = np.cumsum(scan_counts)
        assert len(splits_pc) == len(splits_bev)

        # Concatenate along sequence dimension
        feat = torch.from_numpy(np.concatenate(feat, 0))  # Npc x C
        xyz = torch.from_numpy(np.concatenate(xyz, 0))  # Npc x 3
        ...
        out = {
            ...
            "splits_pc": torch.tensor(scan_counts, dtype=torch.int32),
    ...
    @staticmethod
    def _collate_padded_bev(idx_bev, xy_bev):
        # Modify indices with padding to allow a dense B x sequence x channel tensor.
        idx_occ, idx_pad = [], []
        ...
        return idx_bev, xy_bev, idx_occ, idx_pad, splits_bev
