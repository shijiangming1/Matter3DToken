...
def get_datasets(config, args):
    ...
    kwargs = {
        ...
        "instance_cache_dir": args.instance_cache_dir,
        "prepare_instance_cache": args.prepare_instance_cache,
    }
...
def get_dataloader(train_dataset, val_dataset, args, sparse_batch_reindex=False):
    if args.distributed:
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True,
        collate_fn=Collate(sparse_batch_reindex=sparse_batch_reindex),
    )
...
def distributed_training(gpu, ngpus_per_node, args, config):
    ...
    model = Matter3DToken(**args_model)
    ...
    # Transfer experiments load only network weights. Optimizer, scheduler,
    # scaler, and epoch counters intentionally start fresh.
    if args.init_checkpoint:
        rank = 0 if args.gpu is None else args.gpu
        init_ckpt = torch.load(
            args.init_checkpoint,
            map_location=f"cuda:{rank}",
            weights_only=False,
        )
        model.load_state_dict(init_ckpt["net"])
        print(f"Network weights initialized from: {args.init_checkpoint}")

    freeze_modules = config["optim"].get("freeze_modules", [])
    if freeze_modules:
        ...
        for module_name, module in base_model.named_modules():
            if any(
                ...
                if module_name and module_name not in frozen_names:
                    frozen_names.append(module_name)
        print(f"Frozen module prefixes: {freeze_modules}")
    ...
    mng = TrainingManager(
        ...
        max_train_batches=config.get("debug", {}).get("max_train_batches"),
        max_val_batches=config.get("debug", {}).get("max_val_batches"),
    )
    if (args.restart or args.eval in ["last", "best"]) and not (
        args.init_checkpoint and args.eval in ["last", "best"]
    ):
...
def main(args, config):
    ...
    args.dist_url = os.environ.get("MATTER3DTOKEN_DIST_URL", "tcp://127.0.0.1:4444")
    # Distributed backend'
    ...
    if args.prepare_instance_cache:
        get_datasets(config, args)
        print("Instance cache preparation finished.")
        return
...
def get_default_parser():
    ...
    parser.add_argument(
        "--instance_cache_dir",
        default=os.environ.get("VAVIT_INSTANCE_CACHE_DIR", "data/kitti_instances"),
        help="Local cache for SemanticKITTI InstanceCutMix objects",
    )
    parser.add_argument(
        "--prepare_instance_cache",
        action="store_true",
        help="Build the local SemanticKITTI InstanceCutMix cache and exit",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--init_checkpoint",
        default="",
        help="Initialize network weights only; optimizer and scheduler are reset",
    )
    parser.add_argument(
